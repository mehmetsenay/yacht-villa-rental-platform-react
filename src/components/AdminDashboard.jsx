...
const AdminDashboard = () => {
    ...
    const handleUpdateStatus = async (bookingId, newStatus) => {
        const token = localStorage.getItem('adminToken');
        try {
            const res = await fetch(`${getBackendUrl()}/bookings/${bookingId}/status`, {
                method: 'PATCH',
                headers: {
                                        ...
                                        <tbody>
                                            {bookings.map((booking) => (
                                                <tr key={booking.id} style={styles.tr}>
                                                    ...
                                                    <td style={styles.td}>
                                                        <select
                                                            ...
                                                            style={{
                                                                ...
                                                                borderRadius: '100px',
                                                                border: '1.5px solid',
                                                                borderColor: booking.status === 'CONFIRMED' ? '#34c759' : booking.status === 'CANCELLED' ? '#ff3b30' : '#ff9f0a',
                                                                background: booking.status === 'CONFIRMED' ? '#e4f7e6' : booking.status === 'CANCELLED' ? '#ffe5e5' : '#fff4ce',
                                                                color: booking.status === 'CONFIRMED' ? '#1a7f37' : booking.status === 'CANCELLED' ? '#c0392b' : '#b45309',
                                                                fontSize: '0.85rem',
                                                                fontWeight: 600,
                                                                cursor: 'pointer',
                                                                outline: 'none',
                                                    ...
                                                    <td style={styles.td}>
                                                        <a
                                                            href={`https://wa.me/${(booking.customerPhone || '').replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Merhaba ${booking.customerName}, ${booking.property?.name || 'mülkünüz'} için yaptığınız rezervasyon talebi hakkında görüşmek istiyorum.`)}`}
                                                            target="_blank"

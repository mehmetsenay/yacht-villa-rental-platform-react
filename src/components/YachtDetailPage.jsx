...
import { Wifi, Monitor, Wind, Users, Check, Calendar, Anchor, Compass, Phone, Mail, Globe, Map, Ship, X, Plane, Utensils, Star } from 'lucide-react';
...
const YachtDetailPage = () => {
  ...
  const allAmenities = [
    { icon: <Wifi size={20} />, label: t('filters.amenity_wifi') },
    { icon: <Ship size={20} />, label: 'Güverte Jakuzisi' }, // Missing key
    { icon: <Wind size={20} />, label: t('filters.amenity_ac') },
    { icon: <Users size={20} />, label: 'Mürettebat Hizmeti' },
    { icon: <Monitor size={20} />, label: 'Eğlence Sistemi' },
    { icon: <Utensils size={20} />, label: 'Tam Donanımlı Mutfak' },
    { icon: <Anchor size={20} />, label: 'Su Oyuncakları' },
    { icon: <Compass size={20} />, label: 'Navigasyon Sistemi' },
    { icon: <Plane size={20} />, label: 'Helikopter Pisti' },
    { icon: <Check size={20} />, label: 'Güvenlik Ekipmanları' },
  ];
  const { id } = useParams();
  ...
  const mapContent = React.useMemo(() => {
    ...
    return (
      <iframe
        ...
        marginHeight="0"
        marginWidth="0"
        src={`https://maps.google.com/maps?q=${yacht.latitude || 36.756285},${yacht.longitude || 28.939226}&hl=tr&z=15&output=embed`}
        style={{ filter: 'grayscale(0.2) contrast(1.1)' }}
        title="Yacht Location"
      ></iframe>
    );
  }, [yacht?.latitude, yacht?.longitude]);
  ...
  useEffect(() => {
    ...
    const fetchYacht = async () => {
      try {
        ...
        const transformedData = {
          ...data,
          amenities: allAmenities.slice(0, 6), // Should ideally come from API
          ...
          gallery: data.images && data.images.length > 0
            ...
            : [yachtImg, yachtImg, yachtImg, yachtImg, yachtImg], // Fallback
          // Ensure numeric values/defaults
          rating: 5.0, // Mock
          reviews: 0,
          guests: 10,
          cabins: 5,
        ...
        setYacht(transformedData);
      } catch (err) {
        console.error(err);
        setError('Yat detayları şu anda görüntülenemiyor.');
      } finally {
        setLoading(false);
      }
    };
    ...
    const fetchSimilarYachts = async () => {
      try {
        ...
        const otherYachts = data.filter(p => String(p.id) !== String(id) && p.type === 'YACHT');
        setSimilarYachts(otherYachts);
      } catch (err) {
        console.error("Error fetching similar yachts:", err);
      }
    };
  ...
  const calculateTotal = () => {
    ...
    const basePrice = nights * yacht.dailyPrice;
    let extrasPrice = 0;
    ...
    return {
      nights,
      basePrice, // Reflects dailyPrice * nights
      extrasPrice,
      ...
      <div className="container" style={{ paddingTop: 0, paddingBottom: 80 }}>
        ...
        <div className="gallery-section">
          <div className="gallery-grid">
            {yacht.gallery.slice(0, 5).map((img, idx) => (
              <div
                key={idx}
          ...
          <div className="main-info">
            ...
            <div className="stats-bar">
              <div className="stat-item">
                <div className="stat-value">{yacht.cabins}</div>
                <div className="stat-label">{t('details.cabin')}</div>
              </div>
              <div className="stat-divider"></div>
              <div className="stat-item">
                <div className="stat-value">{yacht.guests}</div>
                <div className="stat-label">{t('details.guest')}</div>
              </div>
              <div className="stat-divider"></div>
              <div className="stat-item">
                <div className="stat-value">{yacht.crew}</div>
                <div className="stat-label">{t('details.crew')}</div>
              </div>
            </div>
            ...
            <div className="description-section">
              ...
              <p className="text-secondary">{yacht.description}</p>
            </div>
            ...
            <div className="location-section">
              ...
              <div className="location-grid">
                <div className="map-container">
                  {mapContent}
                  <a
                    href={`https://maps.google.com/?q=${yacht.latitude || 36.756285},${yacht.longitude || 28.939226}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="map-overlay-btn"
                  >
                    {t('details.google_maps')}
                  </a>
                </div>
                ...
                <div className="distances-list">
                  <div className="distance-item">
                    ...
                    <div className="dist-info">
                      ...
                      <span className="dist-value">25 km</span>
                    </div>
                  </div>
                  <div className="distance-item">
                    <div className="dist-icon"><Anchor size={20} /></div>
                    <div className="dist-info">
                      <span className="dist-label">{t('details.dist_marina')}</span>
                      <span className="dist-value">0 m</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="sidebar-wrapper">
            <div className="booking-card card">
              <div className="booking-header">
                <span className="price">{yacht.dailyPrice.toLocaleString(i18n.language === 'tr' ? 'tr-TR' : 'en-US')} ₺</span> <span className="period">/ {t('booking.per_day')}</span>
              </div>
                ...
                {totals && (
                  <div className="price-breakdown">
                    <div className="breakdown-row">
                      <span>{yacht.dailyPrice.toLocaleString(i18n.language === 'tr' ? 'tr-TR' : 'en-US')} ₺ x {totals.nights} {t('booking.per_night')}</span>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.ok) {
                        // Success Screen (Simple replacement for now)
                        document.body.innerHTML = `
                                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center;">
                                    <h1 style="font-size:3rem; margin-bottom:20px;">${t('booking.success_title')}</h1>
                                    <p style="font-size:1.5rem; color: #666;">${t('booking.success_message')}</p>
                                    <a href="/" style="margin-top:40px; text-decoration:none; color:white; background:black; padding:15px 30px; border-radius:100px;">${t('booking.back_home')}</a>
                                </div>
                             `;
                      } else {
        ...
        <div className="similar-section">
          <h2 className="title-md" style={{ marginBottom: 30 }}>{t('details.similar_yachts')}</h2>
                ...
                >
                  ...
                  <div className="card-details">
                    ...
                    <div className="tags-row">
                      <span className="tag-badge">{item.type === 'YACHT' ? 'Yat' : item.type === 'VILLA' ? 'Villa' : item.type}</span>
                    </div>
